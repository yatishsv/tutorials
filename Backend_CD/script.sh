...
tg_arn=$(aws elbv2 describe-target-groups --names $tg_name --query "TargetGroups[*].TargetGroupArn" | jq -r .[])
...
tgtotal=$(aws elbv2 describe-target-health  --target-group-arn $tg_arn --query 'TargetHealthDescriptions[*].Target.Id' | jq length)
...
#            sleep 60
#            let tgcurrent=$(aws elbv2 describe-target-health  --target-group-arn $tg_arn --query 'TargetHealthDescriptions[*].Target.Id' | jq length)
...
#
#        status=$(aws ssm get-command-invocation --command-id  $commandID --instance-id $target| jq -r '.Status')
#        echo "status = $status"
#
...
#
#        status2=$(aws ssm get-command-invocation --command-id  $commandID2 --instance-id $target| jq -r '.Status')
#
...
#
        while true; 
        do
            read -p "Check the updates and confirm if you want continue (Y/N) ?" yn
                case $yn in
                    [Yy]* ) break;;
                    [Nn]* ) exit;;
